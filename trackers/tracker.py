...
class Tracker:
    def __init__(self, model_or_path):
        # Accept either a YOLO model path or a Roboflow model object
        if hasattr(model_or_path, 'predict') and not isinstance(model_or_path, str):
            # Roboflow model object
            self.model = model_or_path
            self.is_roboflow = True
        else:
            # YOLO model path
            from ultralytics import YOLO
            self.model = YOLO(model_or_path)
            self.is_roboflow = False
        self.tracker = sv.ByteTrack()  #ByteTrack from supervision library
    ...
    def interpolate_ball_positions(self, ball_positions):
        ...
        #interpolate missing values in frames
    ...
    def detect_frames(self, frames):
        ...
        detections = []
        if self.is_roboflow:
            # Roboflow model does not support batch, so process one frame at a time
            for frame in frames:
    ...
    def get_object_tracks(self, frames, read_from_stub=False, stub_path=None):
        ...
        if read_from_stub and stub_path is not None and os.path.exists(stub_path):
            ...
            return tracks #saves time essentially like a cache
        ...
        for frame_num, detection in enumerate(detections):
            ...
            for frame_detection in detection_supervision:
                ...
                if cls_id == cls_names_inv["ball"]:
                    tracks["ball"][frame_num][1] = {"bbox":bbox}  #only one ball, hard code 1
    ...
    def draw_team_ball_control(self,frame,frame_num,team_ball_control):
        #draw semi transparent rectangle at bottom right of screen to display possession statistics
        overlay = frame.copy()
